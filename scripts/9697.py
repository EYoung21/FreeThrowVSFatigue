...
with open(os.path.join(data_dir, 'attempt_counter_1998-1999.txt'), 'r') as f:
    attempts_data = json.load(f)
    
with open(os.path.join(data_dir, 'minute_averages_1998-1999.txt'), 'r') as f:
    minute_averages = json.load(f)
    
with open(os.path.join(data_dir, 'yearly_averages_1998-1999.txt'), 'r') as f:
...
diff_df.to_csv(os.path.join('dataForEachYear', 'difference_averages_1998-1999.txt'), index=False)

# Calculate regression statistics
slope, intercept, r_value, p_value, std_err = stats.linregress(ft_percentages, yearly_percentages)

# Save regression stats
with open('1998-1999_regression_stats.txt', 'w') as f:
    f.write("Analysis for 1998-1999 NBA Seasons\n")
    f.write("="*40 + "\n\n")
...
regression_df.to_csv('1998-1999_regression_analysis.csv', index=False)
...
df.to_csv('1998-1999_ft_percentage_data.csv', index=False)
...
plt.legend()
plt.tight_layout()
plt.savefig('1998-1999_regression_plot.png', bbox_inches='tight', dpi=300)
plt.close()
...
plt.title(f'Free Throw Percentage by Minutes Played for 1998-1999 Season\nFTA: {total_attempts}, FTs Made: {int(total_makes)}, %: {percentage}%', 
          fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig('1998-1999_ft_percentage_analysis.png', bbox_inches='tight', dpi=300)
plt.close()